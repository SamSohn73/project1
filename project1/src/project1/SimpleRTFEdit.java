...
public class SimpleRTFEdit extends JFrame
{
	 ...
	 * Then the window takes care of itself until the program is ended with the Quit
	 * command or when the user closes the window.
	 */
	...
	/**
	 * Create a SimpleRTFEdit window, with an edit pane where the user can edit some
	 * text and with a menu bar. The content type for the text files that are edited
	 * by this program is "text/rtf".
	 */
	public SimpleRTFEdit()
	{
		...
		editPane.setMargin(new Insets(0, 0, 0, 0)); // Some space around the text.
		editPane.setLayout(new BorderLayout(0, 0));
	 ...
	 * File menu contains four commands, New, Open, Save, and Quit. The commands in
	 * the Edit menu are standard editing actions defined in the classes
	...
	/**
	 * Create a KeyStroke that uses the meta key on Mac OS and the control key on
	...
	/**
	 * Carry out the "New" command from the File menu by clearing all the text from
	 * the JTextArea. Also sets the title bar of the window to read "SimpleRTFEdit:
	 * Untitled".
	 */
	private void doNew()
	{
